#include "frame.h"

/*各个数据库的周期更新的QbyteArray*/
//...table.cpp中应该保存所有重要的周期更新的数据
REQUEST dataRx;     //解析用临时全局变量

/*接收和发送缓冲区链表
  接收和发送进程全部都在缓冲区进行操作
  接收：随机产生，有tcpsocket类中的函数执行
  发送：周期产生，有tcpsocket类中的函数执行
  操作类： class table class socket
*/
QList <RX_DATA_ITEM> rxBufferList;
QList <TX_DATA_ITEM> txBufferList;

QList <RX_DATA_ITEM> * GetRxBuffer()
{
    return &rxBufferList;
}
QList <TX_DATA_ITEM> * GetTxBuffer()
{
    return &txBufferList;
}

/*注册指纹相关 临时存储的链表*/
QList <FINGER_DATA_ITEM> fingerBuffer;

QList <FINGER_DATA_ITEM> * GetFingerBuffer()
{
    return &fingerBuffer;
}

//CRC 高字节
const uint8_t auchCRCHi[] = {

};
//CRC 低字节
const uint8_t auchCRCLo[]={

};
/*crc16*/
uint16_t Get_Crc16(uint8_t *puchMsg,uint16_t usDataLen)
{
    uint8_t uchCRCHi=0xFF; 	//¸ßCRC ×Ö½Ú³õÊ¼»¯
    uint8_t uchCRCLo=0xFF; 	//µÍCRC ×Ö½Ú³õÊ¼»¯
    uint32_t uIndex; 		//CRC Ñ­»·ÖÐµÄË÷Òý
    while(usDataLen--) 	//´«ÊäÏûÏ¢»º³åÇø
    {
            uIndex = uchCRCHi^(*puchMsg); //¼ÆËãCRC
            puchMsg++;
            uchCRCHi = uchCRCLo^auchCRCHi[uIndex];
            uchCRCLo = auchCRCLo[uIndex];
    }
    return (uchCRCHi<<8|uchCRCLo);
}
/*crc8*/
uint8_t Get_Crc8(uint8_t *ptr,uint16_t len)
{
    uint8_t crc;
    uint8_t i;
    crc=0;
    while(len--)
    {
        crc^=*ptr++;
        for(i=0;i<8;i++)
        {
            if(crc&0x01)crc=(crc>>1)^0x8C;
            else crc >>= 1;
        }
    }
    return crc;
}


/*注册指纹相关
QList <FINGER_DATA_ITEM> fingerBuffer;

QList <FINGER_DATA_ITEM> * GetFingerBuffer()
*/


